...
typedef enum
{
	USART_OVERSAMPLE_16 = (uint32_t)(0),
	USART_OVERSAMPLE_8	=(uint32_t)(1<<15)
}USART_OVERSAMPLE_E;

typedef enum
{
	USART_DMA_NONE				=(uint32_t)(0),
	USART_DMA_ENABLE_TRANSMITTER=(uint32_t)(1<<7),
	USART_DMA_ENABLE_RECIVER 	=(uint32_t)(1<<6),
	USART_DMA_ENABLE_TX_RX      =(uint32_t)((1<<7)|(1<<6))
...
#define DIV_Fraction(_PCLK_, _BAUD_)						(uint32_t) (((USARTDIV_MUL100(_PCLK_, _BAUD_) -  Mantissa_MUL100(_PCLK_, _BAUD_) ) * OVER_Sample ) / 100 )
...
typedef struct
{
	...
	USART_OVERSAMPLE_E	OVER8 ;				//Over sampling mode default 16 Over Sampling
											// note If use over sampling by 8 you should change @ref OVER_Sample

	USART_DMA_STATUE_E	DMA_ENABLE ; 		// DMA enable transmitter OR DMA enable receiver
