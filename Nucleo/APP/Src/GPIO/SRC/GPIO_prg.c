...
#include "GPIO/INC/GPIO_int.h"
#include "GPIO/INC/GPIO_prv.h"

static GPIO_REG_S* GPIO_PORT [GPIO_PERIPHERAL_NUM] = {GPIOA,GPIOB,GPIOC,GPIOD,GPIOE,GPIOF,GPIOG, GPIOH} ;
...
uint8_t GPIO_u8PinInit(const PinConfig_t* copy_ePinConfig)
{
	...
	if (copy_ePinConfig != NULL)
	{
		if ((copy_ePinConfig->PIN_Port < INVALID) && (copy_ePinConfig->PIN_Number <= PIN15))
		{
			...
			GPIO_PORT[copy_ePinConfig->PIN_Port]->MODER.Reg &= ~(MODER_MASK				     << (copy_ePinConfig->PIN_Number * MODER_PIN_ACCESS)); // clear mode bits
			GPIO_PORT[copy_ePinConfig->PIN_Port]->MODER.Reg |=  ((copy_ePinConfig->PIN_Type) << (copy_ePinConfig->PIN_Number * MODER_PIN_ACCESS));
			/* Select GPIO pull state: PULLUP - PULLDOWN - NOPULL */
			GPIO_PORT[copy_ePinConfig->PIN_Port]->PUPDR.Reg &= ~(PUPDR_MASK				    	    << (copy_ePinConfig->PIN_Number * PUPDR_PIN_ACCESS)); // clear mode bits
			GPIO_PORT[copy_ePinConfig->PIN_Port]->PUPDR.Reg |=  ((copy_ePinConfig->PIN_PullUpDown)  << (copy_ePinConfig->PIN_Number * PUPDR_PIN_ACCESS));
			...
			if ((copy_ePinConfig->PIN_Type == OUTPUT) || (copy_ePinConfig->PIN_Type == ALTERNATE))
			{
				...
				GPIO_PORT[copy_ePinConfig->PIN_Port]->OTYPER.Reg &= ~(OTYPER_MASK				 << (copy_ePinConfig->PIN_Number)); // clear mode bits
				GPIO_PORT[copy_ePinConfig->PIN_Port]->OTYPER.Reg |=  (copy_ePinConfig->PIN_OType << (copy_ePinConfig->PIN_Number));
				/* select speed type LOW_SPEED - MEDIUM_SPEED - FAST_SPEED - HIGH_SPEED */
				GPIO_PORT[copy_ePinConfig->PIN_Port]->OSPEEDR.Reg &= ~(OSPEEDR_MASK				   << (copy_ePinConfig->PIN_Number * OSPEEDR_PIN_ACCESS)); // clear mode bits
				GPIO_PORT[copy_ePinConfig->PIN_Port]->OSPEEDR.Reg |=  (copy_ePinConfig->PIN_OSpeed << (copy_ePinConfig->PIN_Number * OSPEEDR_PIN_ACCESS));
				/* select AF PIN */
...
uint8_t GPIO_vSetOutputPinValue(GPIO_Port_t copy_eGPIO_Port,GPIO_Pin_t copy_eGPIO_Pin,GPIO_ODR_t copy_GPIO_ODR)
{
	...
	if ((copy_eGPIO_Port < INVALID) && (copy_eGPIO_Pin <= PIN15))
	{
		...
		{
			GPIO_PORT[copy_eGPIO_Port]->ODR.Reg &= ~(ODR_MASK			 << (copy_eGPIO_Pin)); // clear mode bits
			GPIO_PORT[copy_eGPIO_Port]->ODR.Reg |=  (copy_GPIO_ODR 	 	 << (copy_eGPIO_Pin));
		}
		else
		{
			Local_u8ErrorState = NOK;
...
uint8_t GPIO_vToggleOutputPinValue(GPIO_Port_t copy_eGPIO_Port,GPIO_Pin_t copy_eGPIO_Pin)
{
	...
	if ((copy_eGPIO_Port < INVALID) && (copy_eGPIO_Pin <= PIN15))
	{
		...
		TOGGLE_BIT(GPIO_PORT[copy_eGPIO_Port]->ODR.Reg ,copy_eGPIO_Pin);
	}
	else
	{
		Local_u8ErrorState = NOK;
...
uint8_t MGPIO_vReadInputPinValue(GPIO_Port_t copy_eGPIO_Port,GPIO_Pin_t copy_eGPIO_Pin,GPIO_ODR_t* copy_GPIO_ODR )
{
	...
	if ((copy_eGPIO_Port < INVALID) && (copy_eGPIO_Pin <= PIN15))
	{
		*copy_GPIO_ODR =READ_BIT(GPIO_PORT[copy_eGPIO_Port]->ODR.Reg ,copy_eGPIO_Pin);;
	}
	else
	{
		Local_u8ErrorState = NOK;
