...
#define RCC_USART1_CLCK_EN()		(RCC->APB2ENR.Reg |=1<<4)
...
UART_ERR_Statue USART_Init( USART_HandleTypeDef* Huart)
{
	...
	uint32_t  LocalU32_BRRValue ;

	/*For Other Configuration*/
	Global_USART_CONFIG=Huart;

	/*For Safety Mask Registers To Reset value*/
	USART[Huart->USART_SELECTION]->CR1 &= ~(0xff);
	USART[Huart->USART_SELECTION]->CR2 &= ~(0xff);
	USART[Huart->USART_SELECTION]->CR3 &= ~(0xff);
	USART[Huart->USART_SELECTION]->BRR &= ~(0xff);
	USART[Huart->USART_SELECTION]->DR  &= ~(0xff);
	...
	{
	case USART_ONE: LocalU32_BRRValue = UART_BRR_Register(8000000 ,Huart->BAUDRATE ); break;
	case USART_TWO:
	case USART_THREE:LocalU32_BRRValue =  UART_BRR_Register(8000000 ,Huart->BAUDRATE );break;
	default : ; break ;
	}
	...
	USART[Huart->USART_SELECTION]->CR3 |=(Huart->DMA_ENABLE);
